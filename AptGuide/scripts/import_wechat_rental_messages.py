...
CREATE_TABLE_SQL = """\
CREATE TABLE IF NOT EXISTS external_rental_listing (
  id BIGINT PRIMARY KEY AUTO_INCREMENT,
  source_type VARCHAR(32) NOT NULL,
  authenticity VARCHAR(32) NOT NULL,
  ...
  rag_visible TINYINT NOT NULL DEFAULT 1,
  appointable TINYINT NOT NULL DEFAULT 0,
  dedupe_key VARCHAR(80) NOT NULL,
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  UNIQUE KEY uk_external_rental_listing_source_hash (source_message_hash),
  KEY idx_external_rental_listing_city_district (city_name, district_name),
  KEY idx_external_rental_listing_rent (rent_min, rent_max),
  KEY idx_external_rental_listing_dedupe (dedupe_key)
);"""
...
def listing_to_insert_sql(listing: dict) -> str:
    ...
    return (
        f"INSERT INTO external_rental_listing ({', '.join(columns)}) VALUES ({values}) "
...
def main() -> int:
                ...
                raw_samples.append(
                    {
                        ...
                        "sender_alias_hash": sha256_text(block.sender_alias),
                        "message_type": block.message_type,
                        "raw_content_redacted": redact_sensitive_text(block.body),
    ...
    listings = sorted(
        ...
        key=lambda item: (item.get("district_name") or "", item.get("rent_min") or 0, item["source_message_hash"]),
    )
    write_jsonl(output_dir / "wechat_rental_raw_messages_sample.jsonl", raw_samples)
    write_jsonl(output_dir / "wechat_rental_listings_sanitized.jsonl", listings)

    with (output_dir / "wechat_rental_listings_seed.sql").open("w", encoding="utf-8") as file:
        file.write("-- WeChat real rental listing import. Local/test only. Sanitized external leads.\n")
